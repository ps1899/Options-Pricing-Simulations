...
from OptionPricingMethod import OptionPricingMethod
...
class OptionPricingMethod(Enum):
    ...
    def getHistoricalData(ticker):
        data = Ticker.getHistoricalData(ticker)
        if data is None:
            st.error("Failed to fetch historical data for the respective ticker!!")
            return pd.DataFrame()
        return data
    ...
    if(__name__ == '__main__'):
        ...
        pricingMethod = st.sidebar.radio('Please select option pricing method', options = [model.value for model in OptionPricingMethod])

        st.subheader(f'Pricing method: {pricingMethod}')

        if(pricingMethod == OptionPricingMethod.blackScholes.value):
        ...
        elif(pricingMethod == OptionPricingMethod.monteCarlo.value):
        ...
        elif(pricingMethod == OptionPricingMethod.binomialTree.value):
